...
const BlogsList = () => {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => api.get(`/blogs?search=${searchTerm}&limit=100`).then(res => res.data)
  });
  ...
  return (
    <div>
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-gray-200">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {data?.blogs?.length === 0 ? (
                  <tr>
                    <td colSpan="4" className="px-6 py-8 text-center text-gray-500">No blogs found.</td>
                  </tr>
                ) : (
                  data?.blogs?.map((blog) => (
                    <tr key={blog.Id} className="hover:bg-gray-50">
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex items-center">
                          <div className="h-10 w-10 rounded-lg overflow-hidden bg-gray-100 flex-shrink-0">
                            {blog.screens ? (
                              <img src={blog.screens} alt="" className="h-full w-full object-cover" />
                            ) : (
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                        <button onClick={() => openModal(blog)} className="text-indigo-600 hover:text-indigo-900 mr-4">
                          <Edit2 size={16} />
                        </button>
                        <button onClick={() => handleDelete(blog.Id || blog.slug)} className="text-red-600 hover:text-red-900">
                          <Trash2 size={16} />
                        </button>
                      </td>
                    </tr>
      ...
      {isModalOpen && (
        <BlogModal blog={editingBlog} onClose={closeModal} />
      )}
    </div>
  );
};
...
const BlogModal = ({ blog, onClose }) => {
  ...
  const { register, handleSubmit, control, formState: { errors } } = useForm({
    defaultValues: blog || {
      ...
      author: 'Admin',
      authorTitle: 'Author',
      createAt: new Date().toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }),
      isPublished: true
    }
  });
  ...
  const mutation = useMutation({
    mutationFn: (data) => {
      if (blog) {
        return api.put(`/blogs/${blog.Id || blog.slug}`, data);
      } else {
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="px-4 py-5 sm:p-6 space-y-4 max-h-[70vh] overflow-y-auto">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="md:col-span-2">
                <label className="block text-sm font-medium text-gray-700">Blog ID (Unique)</label>
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700">Category / Thumb</label>
                <input {...register('thumb', { required: true })} className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" />
              </div>
              
              <div>
                <label className="block text-sm font-medium text-gray-700">Author</label>
                <input {...register('author', { required: true })} className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" />
              </div>

              <div>
                <label className="block text-sm font-medium text-gray-700">Author Title</label>
                <input {...register('authorTitle', { required: true })} className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" />
              </div>

              <div className="md:col-span-2">
                <label className="block text-sm font-medium text-gray-700">Description (Summary)</label>
                <textarea {...register('description')} rows={2} className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" />
              </div>

              <div className="md:col-span-2">
                <label className="block text-sm font-medium text-gray-700">Content (HTML)</label>
                <textarea {...register('bSingle')} rows={6} className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm font-mono text-sm" />
              </div>
              ...
              <div className="md:col-span-2">
                <div className="flex items-center">
                  <input id="isPublished" type="checkbox" {...register('isPublished')} className="h-4 w-4 text-indigo-600 focus:ring-indigo-500 border-gray-300 rounded" />
